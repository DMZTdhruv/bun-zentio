  ...
  withRef<typeof PlateElement>(({ children, className, ...props }, ref) => {
    ...
    const content = (
      <PlateElement
        className={cn(
          className,
          "overflow-x-auto bg-transparent py-5",
          hasControls && "-ml-2 *:data-[slot=block-selection]:left-2",
        )}
        style={{ paddingLeft: marginLeft }}
        {...props}
      >
        <div className="group/table relative w-fit bg-transparent">
          <table
            ref={ref}
            className={cn(
              "mr-0 ml-px table h-px table-fixed border-collapse bg-transparent",
              isSelectingCell && "",
            )}
            {...tableProps}
          >
            <tbody className="min-w-full bg-transparent">{children}</tbody>
          </table>
        </div>
      </PlateElement>
    );
